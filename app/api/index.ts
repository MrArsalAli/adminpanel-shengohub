...
export const getATeachersRequest = async (id: any) => {
  try {
    const response = await axios.get(
      `${AppRoutes.getSingleTeacherRequest}/${id}`,
    );
    return response?.data?.data;
  } catch (error: any) {
    console.error("Error fetching a teacher:", error?.message);
...
export const createTutorPayout = async (teacherId: any) => {
  const response = await axios.post(AppRoutes.createTutorPayout, {
    teacherId,
  });
...
export const adminRefundLessonPackage = async (packageId: any) => {
  const response = await axios.post(AppRoutes.adminRefundLessonPackage, {
    packageId,
  });
